...
getAMerchandise = async (request, response) => {
    ...
    console.log(id)
    try {
        const { rows } = await db.query("SELECT * FROM merchandises WHERE merchan_id=$1", [id])
        console.log(rows)
        return response.status(200).json({ 
             rows
          })
...
createMerchandise = async ( request, response) =>{
   ...
   try {
    await db.query('INSERT INTO merchandises(category, address, image, owner_id) VALUES ($1, $2, $3, $4)', [category, address, image, owner_id])
...
module.exports = {getAllMerchandises, getVendorMerchandises, getAMerchandise, createMerchandise}